...
async def start(client, message):
    ...
    # only insert empty profile if not exists
...
async def profile_steps(client, message):
    ...
    if step == "name":
        ...
        # merge with existing profile instead of overwrite
        user = await db.get_user(user_id)
...
async def search_partner(client, message):
    ...
    if not profile.get("gender") or not profile.get("age") or not profile.get("location"):
        await message.reply_text("⚠️ Please complete your profile first using /profile")
        return
